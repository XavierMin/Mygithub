...
//	u8 send_dat[200];
  ...
  while(1)
    ...
    printf("******************************\r\n" );		
		...
		if(t>=10)//5秒钟发送一次数据，修改此值即可定义上传数据时间
		{
			t=0;
			LED0=!LED0;
			if(link)
			{
				sprintf((char *)send_dat, "%3.1f%3.1f%3.1f", temperature, humidity, result_lx);
				printf( "\r\n" );
        printf( "[INFO]*****************************************\r\n" );
				printf( "[INFO]Coap send-->%s\r\n", send_dat);
				nb_coap_report(send_dat, strlen(send_dat));
				delay_ms(1000);
			}
		}		
		delay_ms(200);
